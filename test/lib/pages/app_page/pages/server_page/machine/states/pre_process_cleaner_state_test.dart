...
class MockPreDuplicateCleanerState extends Mock
    implements PreDuplicateCleanerState {}
...
void main() {
  ...
  late PreDuplicateCleanerState preDuplicateCleanerState;
  late PreProcessCleanerState state;
  ...
  setUp(() {
    ...
    state = PreProcessCleanerState(
      machine,
      processCleanerRepository: repository,
    );
    machineState = MockState();
    idleState = MockIdleState();
    preDuplicateCleanerState = MockPreDuplicateCleanerState();
    when(() => machine.state).thenReturn(machineState);
    when(() => machine.idleState).thenReturn(idleState);
    when(() => machine.preDuplicateCleanerState)
        .thenReturn(preDuplicateCleanerState);
  });

  group('PreProcessCleanerState', () {
    group('start', () {
      ...
      test('transition to preDuplicateCleanerState', () async {
        when(() => machine.projectPath).thenReturn('');
        when(() => repository.killJavaProcesses()).thenAnswer((_) async => {});
        await expectLater(state.start(), completes);
        verify(() => machine.addLog(any())).called(2);
        verify(() => machine.state = preDuplicateCleanerState);
      });
    });
  });
}
